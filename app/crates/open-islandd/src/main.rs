...
fn broadcast(state: &SharedState, message: String) -> bool {
...
fn broadcast_except(state: &SharedState, message: String, except: Option<u64>) -> bool {
...
fn make_broadcast(state: SharedState) -> impl Fn(String) -> bool + Clone {
...
fn make_hook_broadcast(state: SharedState, connection_id: u64) -> impl Fn(String) -> bool + Clone {
...
fn poller(ctx: DaemonContext, flag: ShutdownFlag) {
...
fn usage_poller(ctx: DaemonContext, flag: ShutdownFlag) {
...
fn announce_quiet_scenes(ctx: &DaemonContext, source: &mut dyn SceneSource) {
    let config = ctx.config.get();
