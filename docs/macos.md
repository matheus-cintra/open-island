...
- No Warp, o foco ativa o aplicativo e novas sessões usam configurações de lançamento.
  O envio de mensagens usa uma ponte PTY própria nas sessões abertas pela ilha.
  Para comandos digitados no terminal, habilite a integração de entrada nos ajustes
  e abra uma nova sessão. Veja [mensagens em qualquer terminal](terminal-input.md).
...
No macOS, expandir a ilha não solicita mais uma janela com foco de teclado. O foco
é solicitado ao selecionar um campo e liberado ao sair dele, remover o cartão,
trocar de janela ou recolher a ilha. O Linux mantém sua política de teclado.

Verificação local: 161 testes da interface e build TypeScript/Vite aprovados;
quatro testes Rust de lançamento aprovados. O comportamento físico de abertura no
